...
RSpec.describe Sidekiq::AsyncHttp::HttpError do
  describe ".new factory" do
    it "returns HttpError for non-4xx/5xx status" do
      ...
      expect(error.class).to eq(Sidekiq::AsyncHttp::HttpError)
      expect(error).not_to be_a(Sidekiq::AsyncHttp::ClientError)
      expect(error).not_to be_a(Sidekiq::AsyncHttp::ServerError)
    end
  end
  ...
  describe "#error_class" do
    it "returns self.class" do
      ...
      expect(error.error_class).to eq(Sidekiq::AsyncHttp::ClientError)
    end
  end
end
