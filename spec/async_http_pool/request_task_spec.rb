...
RSpec.describe AsyncHttpPool::RequestTask do
  let(:request) do
    AsyncHttpPool::Request.new(:get, "https://api.example.com/users")
  end
  ...
  describe "#initialize" do
    ...
    it "uses request max_redirects when set" do
      request_with_redirects = AsyncHttpPool::Request.new(:get, "https://api.example.com", max_redirects: 3)
      task = described_class.new(
  ...
  describe "#redirect_task" do
    let(:post_request) do
      AsyncHttpPool::Request.new(:post, "https://api.example.com/submit", body: '{"data":"value"}', timeout: 30)
    end
    ...
    it "preserves max_redirects setting from request" do
      request_with_max = AsyncHttpPool::Request.new(:get, "https://api.example.com/users", max_redirects: 3)
      task = described_class.new(
