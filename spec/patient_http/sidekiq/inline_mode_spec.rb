...
RSpec.describe "Sidekiq::Testing.inline! mode" do
  ...
  describe "successful HTTP request" do
    it "executes HTTP request inline and calls callback inline" do
      # Stub HTTP request
      stub_request(:get, "https://api.example.com/users")
        .to_return(status: 200, body: '{"users": []}', headers: {"Content-Type" => "application/json"})

      template = PatientHttp::RequestTemplate.new(base_url: "https://api.example.com")
      ...
      # Verify callback on_complete was called inline (not enqueued)
  ...
  describe "failed HTTP request with error callback" do
    it "executes HTTP request inline and calls on_error inline on connection error" do
      ...
      # Verify on_error was called inline
  ...
  describe "with custom headers" do
    it "includes custom headers in the inline request" do
      ...
      request = PatientHttp::Request.new(:get, "https://api.example.com/secure", headers: {"Authorization" => "Bearer token123"})
