...
module Sidekiq::AsyncHttp
  ...
  class ClientPool
    ...
    def make_proxied_client(endpoint)
      ...
      # Use the proxy method from Proxy::Client module (prepended to Async::HTTP::Client)
      # This creates a Proxy instance with the target's authority
      proxy = @proxy_client.proxy(configured_endpoint)

      # wrap_endpoint creates an endpoint that routes through the proxy
